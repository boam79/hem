...
03 시안은 풀블리드 HUD가 아니다. 연회색 페이지 + 흰 헤더 + 왼쪽 안건/업로드 카드 + 오른쪽 라운드 회의실이다. 카드를 장면 위에 올리면 왼쪽 사람·CTA가 가려진다. `object-position` 70%대는 테이블만 보여 머리를 자른다. 분할 레이아웃이면 3:2 장면도 머리와 테이블이 같이 들어온다.
