...
2026-08-29 종이더미: 위치 61.5% 유지. 대기는 불투명 1장(바닥은 그대로). 업로드·토론 시작 시 그 위에 시트가 로딩바처럼 떨어지고 위로 갈수록 불투명이 진해진다. 점토 PNG 전체를 맞추지 않는다.
...
짧은 더미를 만들려 `height: 235%` + `overflow: hidden`으로 PNG 아래만 확대하면 윗장이 가로로 잘린다. 위치는 `translate(-50%, -100%)`로 바닥을 고정하고, 대기 때는 불투명 1장, 업로드·토론 시작 때만 시트를 로딩바처럼 쌓는다. 점토 PNG 전체를 맞추려 하지 않는다.
