...
### 3.2 제외 (정식 서비스 시)
자동 메모 초안, 라운드 3, 단일 모델 비교 모드, 인증·RLS, 실데이터 업로드

### 3.3 2026-08-29 제품 확장
사용자 요청으로 아래를 포함한다.

- 설정: 페르소나별 Boardroom 토큰·추정 비용, 월 예산 대비 잔여, 각 사 콘솔 링크
- 페르소나 편집: 이름·역할·습관·temperature. 프로바이더·modelId는 F5로 코드 고정
- 대시보드: 비용 대시보드
- 홈 라운드: `POST /api/round/stream` SSE 토큰 스트리밍. 기존 `POST /api/round` JSON은 유지
