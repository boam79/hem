...
2026-09-14: 사용자는 올린 병원 지표가 「토론 시작」에 안 들어간다고 했다. 선택 창의 「그냥 진행」이 기본 합성 지표만 써서 그렇다. 유형(마케팅·인력·가격)은 DB 태그일 뿐 가중치가 아니다. 파일이 있으면 토론 시작이 올린 지표를 바로 넣고, 유형 UI는 뺀다.
...
사이드바 6칸을 더 늘리면 02 셸이 답답해진다. 통계는 올린 직후 `/files`에 표를 붙이고, 안건 없는 회의는 홈 「데이터 검토」만 추가한다. 인사이트용 4번째 LLM은 두지 않는다. 「토론 시작」의 안건 10자 규칙은 유지한다. 데이터 검토는 고정 안건을 POST에만 넣는다. E2E는 「데이터 검토」와 선택 창의 「올린 데이터로 진행」「그냥 진행」을 누르지 않는다. 올린 파일이 있으면 「토론 시작」이 바로 LLM을 부르므로 E2E는 그 경우 「토론 시작」을 누르지 않는다. 파일 없을 때만 선택 창.
헤더 「시뮬레이션 다음 턴」은 「토론 시작」과 같아 지웠다. 「회의 나가기」가 헤더 오른쪽에 있다.
...
2026-09-14 Executor: 올린 지표가 토론에 안 들어가는 문제.
1. 파일 있으면 「토론 시작」= 올린 지표. 유형 UI 제거, category는 `marketing` 고정. 프롬프트에 올린 병원명. 성공: 단위. E2E는 업로드 후 힌트만 보고 「토론 시작」은 누르지 않음.
...
- [ ] 고도화. 08-30 P1–P5 + 09-14 R1–R5·H1·H2. 코드 반영. HTTPS·실토론은 DB 500으로 사용자 확인 대기.
...
- [ ] 헤더 「시뮬레이션 다음 턴」삭제 · 「회의 나가기」를 그 자리로. HTTPS 사용자 확인 대기.
- [ ] 홈 헤더 한 줄 축소 · 전체화면에서 페이지 스크롤 없음. HTTPS 사용자 확인 대기.
...
2026-09-01 Executor: 큰 `2026` 오버레이가 구워진 `2024`와 겹쳐 숫자가 깨짐. 원본 PNG에서 `4`만 지우고 같은 크기 `6`을 그림. `년`·제목은 그대로. 캐시 `03y2026b`. HTTPS 사용자 확인 대기.

2026-09-01 Executor: 「시뮬레이션 다음 턴」은 초기화가 아니라 「토론 시작」과 같은 선택 창. 헤더에서 지움. 「회의 나가기」는 세션·라운드만 비움(안건·파일 유지). 장면 위 떠 있던 버튼을 헤더 오른쪽(옛 다음 턴 자리)로 옮김. LLM 없음.

2026-09-14 Executor: 홈 헤더를 한 줄로 줄임. 셸 `100dvh`로 전체화면에서 페이지 스크롤이 없게. 회의실 비율은 그대로. HTTPS 사용자 확인 대기.

2026-09-14 Executor: 고도화 일괄. H1 빈 R2는 한글 키·위험 문장으로 채움. P1 쟁점 모음, P4 심사 안건, P5 링크 복사, R1 세션 훅, P2·P3 통계 칩, H2 실패 셀 다시, U1 최근 회의. R3 CSS 분리는 안 함. 단위 132·빌드 통과. 프로덕션 세션 GET 500이라 HTTPS 실토론 못 함.
...
03 시안 테이블 서류는 장면 PNG에 구워져 있다. 손익계산서·진료과별 현황 SVG 카드를 그 위에 올리면 시안과 다른 스티커가 된다. 업로드 여부는 `data-stack`만으로 두고, HTML 서류 오버레이는 그리지 않는다. 서류 연도(손익계산서 분기·타당성 검토)는 PNG에 있다. ImageMagick `-draw "text x,y"`의 y는 베이스라인이다. 큰 `2026`을 구워진 `2024` 위에 올리면 4가 비치고 `년`이 뭉갠다. 원본에서 마지막 4만 지우고 6–11px `6`을 그린다. 홈 도크를 지우면 `.table-waiting-label` bottom을 5.6rem에서 내려야 한다. 홈 헤더를 아이콘+글자 2줄로 두면 세로가 ~80px가 되어 `100vh - 6rem`과 어긋나고 전체화면에서도 스크롤이 난다. 메뉴는 한 줄, 셸은 `100dvh` + `overflow: hidden`(960px 이하는 세로 흐름).
