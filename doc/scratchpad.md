...
2026-08-29 종이더미: CSS 사각형은 점토 질감이 안 남. `clay-paper-stack.png`를 쓰고, 대기는 맨 위 한 장(그리드 아이콘), 업로드·토론 시 창이 커지며 아래로 타일이 차오른다. 위치 61.5% 유지.
...
짧은 더미를 만들려 `height: 235%` + `overflow: hidden`으로 PNG 아래만 확대하면 윗장이 가로로 잘린다. rotateX 뒤 translateY로 쌓으면 종이가 테이블 면으로 펼쳐진다. CSS 사각형은 점토 PNG 질감이 안 난다. PNG는 확대하지 않고 위(아이콘 장)부터 창 높이만 키워 타일이 차오르게 한다.
