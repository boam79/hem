...
2026-08-29 종이더미: 위치 61.5% 유지. 대기는 불투명 1장. 업로드·토론 시작 시 **같은 자리**에서 Z축으로 차곡차곡 쌓인다(테이블 면으로 펼치지 않음). 점토 PNG 전체를 맞추지 않는다.
...
짧은 더미를 만들려 `height: 235%` + `overflow: hidden`으로 PNG 아래만 확대하면 윗장이 가로로 잘린다. rotateX 뒤 translateY로 쌓으면 종이가 테이블 면으로 펼쳐진다. 쌓임은 translateZ로 같은 자리에 올린다. 점토 PNG 전체를 맞추려 하지 않는다.
