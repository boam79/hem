...
2026-08-29 종이더미: 타워 PNG를 잘라 쓰면 대기에도 여러 장처럼 보임. `clay-paper-tile.png` 한 장을 대기 1장, 업로드·토론 시 같은 타일을 11px씩 올려 차곡차곡 떨어뜨림. 위치 61.5%.
...
짧은 더미를 만들려 `height: 235%` + `overflow: hidden`으로 PNG 아래만 확대하면 윗장이 가로로 잘린다. 타워 PNG 창을 키워도 아이소메트릭 윗면이 여러 장처럼 보인다. 점토 타일 한 장(`clay-paper-tile.png`)을 대기 1장으로 두고, 활동 시 같은 타일을 올려 쌓는다.
